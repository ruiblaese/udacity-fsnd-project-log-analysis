...
def main():
    ...
    top_articles_html = "".join(TOP_ARTICLES_AUTHORS_DIV %
                                (title, views) for title, views in
                                get_top_articles())

    top_authors_html = "".join(TOP_ARTICLES_AUTHORS_DIV %
                               (name, views) for name, views in
                               get_top_authors())

    top_errors_html = "".join(TOP_ERRORS_DIV %
                              (date, errors) for date, errors in
                              get_top_errors())
